...
public class Conta {
    ...
    public void creditar(BigDecimal credito) {
        if (isNull(credito)) obrigatorio("Valor crédito");
        if (credito.compareTo(BigDecimal.ZERO) <= 0) obrigatorio("Valor crédito");

        saldo = saldo.add(credito);
    }

    public void debitar(BigDecimal debito) {
        if (isNull(debito)) obrigatorio("Valor débito");
        if (debito.compareTo(BigDecimal.ZERO) <= 0) obrigatorio("Valor débito");
        if (debito.compareTo(saldo) > 0) saldoInsuficiente();
